...
async def process_slide_deck_endpoint(
    deck_format: str = Form(...),
    file: UploadFile = File(...)
):
    logger.info(
        f"Received request to process slide deck. Format: {deck_format}")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to save the uploaded file.")

    # Process the slide deck
    try:
        logger.info(f"Starting to process slide deck: {file_location}")
        result = await process_slide_deck(file_location)
        if result is None:
            logger.error("Processing failed, result is None")
            raise HTTPException(status_code=500, detail="Processing failed.")

        # Add slide image info to the result
        result.slides = [SlideInfo(slide_number=i+1, image_path=get_slide_image_path(file_location, i+1))
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail="Failed to process the slide deck.")
    finally:
...
async def get_slide_image(slide_number: int):
    ...
    processed_files = [f for f in os.listdir(
        "uploads") if f.endswith(".pptx") or f.endswith(".pdf")]
...
@router.get("/slide-thumbnails/{slide_number}")
async def get_slide_thumbnail(slide_number: int):
    # This assumes that the most recent processed file is the one we want
    # You might need to implement a more robust way to track the current file
    processed_files = [f for f in os.listdir(
        "uploads") if f.endswith(".pptx") or f.endswith(".pdf")]
    ...
    if not os.path.exists(image_path):
        ...
        raise HTTPException(
            status_code=404, detail="Slide thumbnail not found")
